...
class App extends Component {
  ...
  render () {
    ...
    return (
        <Router>
        <Switch>
        {!this.state.session && [<Route exact path ="/register" component = {()=> <Registration onRegister={this.signIn}/>} />,<Route component = {()=> <Login onLogin={this.signIn} history={history}/>} />]}
        {this.state.session && [<Route exact path="/home" component = {() => <HomePage session={this.state.session} />}/>,<Route exact path="/createorder" component = {() => <CreateOrder session={this.state.session} />}/>,<Route exact path="/createproduct" component={CreateProduct} session={this.state.session}/>,<Route exact path="/creategroup" component={CreateProductGroup} session={this.state.session}/>,<Route exact path="/forecast" component={ForecastDetail} session={this.state.session}/>,<Route exact path="/inventory" component={Inventory} session={this.state.session}/>,<Route exact path="/history" component={History} session={this.state.session}/>,<Route component={HomePage} />]}
